 ...
 * preservePath: true permet de préserver les chemins des fichiers
 */
...
const uploadService = {
  ...
  /**
   * Upload pour création de boutique (documents + images optionnels)
   */
  uploadStoreCreation: uploadMemory.fields([
    { name: 'logo', maxCount: 1 },
    { name: 'banner', maxCount: 1 },
...
const uploadMiddleware = (uploadType) => {
  return (req, res, next) => {
    // Debug: Log avant parsing Multer
    console.log('📤 Avant Multer - Content-Type:', req.headers['content-type']);
    console.log('📤 Avant Multer - Method:', req.method);
    console.log('📤 Avant Multer - Path:', req.path);
    console.log('📤 Avant Multer - Body keys:', Object.keys(req.body || {}));
    
    uploadType(req, res, (error) => {
      // Debug: Log après parsing Multer
      console.log('📥 Après Multer - Body keys:', Object.keys(req.body || {}));
      console.log('📥 Après Multer - Body sample:', {
        name: req.body?.name || 'MANQUANT',
        description: req.body?.description ? req.body.description.substring(0, 50) + '...' : 'MANQUANT',
        city: req.body?.city || 'MANQUANT',
        address: req.body?.address || 'MANQUANT'
      });
      console.log('📥 Après Multer - Files:', req.files ? Object.keys(req.files) : 'no files');
      
      if (error instanceof multer.MulterError) {
        console.error('❌ Erreur Multer:', error.code, error.message);
