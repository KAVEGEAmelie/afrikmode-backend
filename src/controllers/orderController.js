...
const createOrder = asyncHandler(async (req, res) => {
  ...
  if (couponCode) {
    try {
      ...
      // Préparer les données de commande pour validation
      const orderData = {
        subtotal: subtotal,
      ...
      if (validationResult.isValid) {
        ...
        // Ajuster les frais de livraison si coupon livraison gratuite
        if (applicationResult.freeShipping) {
          shippingCost = 0;
        }
      } else {
      ...
      console.error('Erreur validation coupon:', couponError);
      return res.status(400).json({
        success: false,
  ...
  try {
      ...
      try {
        ...
        await Coupon.recordUsage(
          ...
          order.id,
          {
            ...
            appliedToItems: validatedItems.map(item => ({
              product_id: item.productId,
              quantity: item.quantity,
        ...
        console.error('Erreur enregistrement utilisation coupon:', couponUsageError);
        // Ne pas faire échouer la commande pour cela, mais logger l'erreur
      }
    }
    ...
    // Envoyer notification push de confirmation (en arrière-plan)
    NotificationHelpers.sendOrderConfirmationNotification(
      {
        id: order.id,
    ...
    // Vérifier et activer les récompenses de parrainage (en arrière-plan)
    ReferralService.activateReferralRewards(order.id, req.user.id, totalAmount)
      .then(result => {
        if (result.success) {
          console.log(`✅ Récompenses de parrainage activées pour commande ${order.order_number}`);
        }
      })
      .catch(error => {
        console.error('Erreur activation récompenses parrainage:', error);
      });
...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  // Récupérer les données utilisateur pour la notification
  const userData = await db('users')
    .where({ id: order.customer_id })
    .select(['id', 'email', 'first_name', 'preferred_language', 'tenant_id'])
    .first();

  // Envoyer notification de mise à jour de statut (en arrière-plan)
  if (userData) {
    NotificationHelpers.sendOrderStatusUpdateNotification(
      {
        id: order.id,
        order_number: order.order_number
      },
      {
        ...
        language: userData.preferred_language || 'fr',
        tenant_id: userData.tenant_id
      },
      status
    ).catch(error => {
      console.error('Erreur envoi notification statut commande:', error);
    });

    // Envoyer demande d'avis si la commande est livrée
    if (status === 'delivered') {
      NotificationHelpers.sendReviewRequestNotification(
        {
          id: order.id,
          order_number: order.order_number
        },
