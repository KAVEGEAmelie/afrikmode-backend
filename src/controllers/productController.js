...
const getAllProducts = asyncHandler(async (req, res) => {
  ...
  // Compter le total sans les colonnes spécifiques pour éviter l'erreur GROUP BY
...
const createProduct = asyncHandler(async (req, res) => {
  ...
  // Envoyer notification de nouveau produit (si le produit est actif)
  if (product.status === 'active' || product.status === 'published') {
    try {
      await NotificationHelpers.sendNewProductNotification(product, category, req.user.tenantId);
      console.log(`✨ Notification nouveau produit envoyée: ${product.name}`);
    } catch (notifError) {
      console.error('Erreur notification nouveau produit:', notifError);
...
/**
 * Fonction utilitaire pour notifier le retour en stock
 * À appeler quand le stock d'un produit passe de 0 à > 0
 */
const notifyBackInStock = async (productId, tenantId) => {
  try {
    ...
    // Récupérer les utilisateurs intéressés par ce produit
    // (dans une vraie app, on aurait une table wishlist ou notifications_interesse)
    const interestedUsers = await NotificationHelpers.getUsersInterestedInProduct(productId, tenantId);
    
    if (interestedUsers.length > 0) {
      await NotificationHelpers.sendBackInStockNotification(product, interestedUsers, tenantId);
      console.log(`🔄 Notification retour en stock envoyée pour: ${product.name}`);
    }

  } catch (error) {
    console.error('Erreur notification retour en stock:', error);
  }
};

/**
 * Fonction utilitaire pour notifier les promotions sur produits
 * À appeler quand un produit est mis en promotion
 */
const notifyProductSale = async (productId, saleData, tenantId) => {
  try {
    ...
    if (interestedUsers.length > 0) {
      await NotificationHelpers.sendProductOnSaleNotification(product, saleData, tenantId);
      console.log(`🔥 Notification promotion envoyée pour: ${product.name}`);
    }

  } catch (error) {
    console.error('Erreur notification promotion produit:', error);
  }
};

/**
 * Fonction pour mettre à jour le stock d'un produit avec notification
 * À utiliser dans les autres contrôleurs (orders, etc.)
 */
const updateProductStock = async (productId, newQuantity, tenantId) => {
  try {
    ...
    // Si le produit était en rupture et revient en stock
    if (oldProduct.stock_quantity === 0 && newQuantity > 0) {
      await notifyBackInStock(productId, tenantId);
    }

    return true;

  } catch (error) {
    console.error('Erreur mise à jour stock produit:', error);
    throw error;
  }
};
