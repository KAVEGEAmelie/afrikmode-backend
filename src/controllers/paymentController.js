 ...
 * Appelée par le paymentService après traitement des webhooks
 */
const handlePaymentNotification = async (paymentData, orderData, userData, status) => {
  try {
    if (status === 'completed' || status === 'paid') {
      // Notification de paiement réussi
      await NotificationHelpers.sendPaymentSuccessNotification(paymentData, orderData, userData);
      console.log(`✅ Notification paiement réussi envoyée pour ${paymentData.amount} FCFA`);
    } else if (status === 'failed' || status === 'declined') {
      // Notification de paiement échoué
      await NotificationHelpers.sendPaymentFailedNotification(paymentData, orderData, userData);
      console.log(`❌ Notification paiement échoué envoyée pour ${paymentData.amount} FCFA`);
    }
  } catch (error) {
