...
const reviewRoutes = require('./reviews'); // Product reviews
const messageRoutes = require('./messages'); // Client-vendor messaging
const vendorRoutes = require('./vendor'); // Vendor space routes
const adminRoutes = require('./admin'); // Admin panel routes
...
router.use('/i18n', i18nRoutes); // Internationalization routes are public
router.use('/currencies', currencyRoutes); // Currency routes are public
router.use('/2fa', twoFactorRoutes); // 2FA routes mix public and private

// Protected routes (authentication required)
router.use('/users', authMiddleware.requireAuth, userRoutes);
router.use('/cart', authMiddleware.requireAuth, cartRoutes);
router.use('/wishlist', authMiddleware.requireAuth, wishlistRoutes);
router.use('/reviews', reviewRoutes); // Mix public (GET) and protected (POST/PUT/DELETE)
router.use('/messages', authMiddleware.requireAuth, messageRoutes); // Client-vendor messaging
...
router.use('/tickets', authMiddleware.requireAuth, ticketRoutes);
router.use('/coupons', authMiddleware.requireAuth, couponRoutes);
router.use('/notifications', authMiddleware.requireAuth, notificationRoutes);
router.use('/referrals', referralRoutes); // Mix of public and protected routes
router.use('/reports', authMiddleware.requireAuth, reportRoutes); // Reports need authentication
router.use('/security', securityRoutes); // Security routes mix public health check and admin features
router.use('/newsletter', authMiddleware.requireAuth, newsletterRoutes); // Newsletter and marketing campaigns
router.use('/advanced', authMiddleware.requireAuth, apiRoutes); // Advanced API features (webhooks, keys)
router.use('/media', authMiddleware.requireAuth, mediaRoutes); // Media management with CDN
router.use('/mobile', mobileRoutes); // Mobile features (deep links public, others protected)
// Vendor space routes (protected, vendor role required)
router.use('/vendor', authMiddleware.requireAuth, vendorRoutes);

// Admin panel routes (protected, admin role required)
